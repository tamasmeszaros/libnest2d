...
struct Func {
    ...
    Func(std::string& s): str(s) {}

    template<class T> void operator()(int idx, T&& input) {
        std::cout << idx << " " << str << " " << input << std::endl;
    }
};
int main(void /*int argc, char **argv*/) {

    opt::TOptimizer<opt::Method::SIMPLEX> optim;

    std::string str = "Hello";

    opt::metaloop::map([str](int idx, auto&& input){
        std::cout << idx << " " << str << " " << input << std::endl;
    }, std::make_tuple(0.4, 5, 'a', 'c', 90u));
